...
async function loadProfileData(userId, query) {
    const now = new Date();
...
function isApiProfilPath(req) {
    return req.originalUrl && req.originalUrl.indexOf('/api/profil') === 0;
}
...
exports.profile = async function (req, res) {
    ...
    try {
        const data = await loadProfileData(userId, req.query);
        ...
        res.redirect('/logowanie?error=Błąd%20ładowania%20profilu');
    }
};

exports.profileApi = async function (req, res) {
    const userId = req.session.user.id;
    ...
    try {
        ...
        console.error('Błąd ładowania profilu:', err);
        res.status(500).json({ message: 'Błąd ładowania profilu' });
    }
};
...
exports.changePassword = async function (req, res) {
    ...
    try {
        ...
        if (users.length === 0) {
            if (api) {
                return res.status(401).json({ success: false, message: 'Sesja nieważna' });
            }
        ...
        if (!isMatch) {
            if (api) {
                return res.status(400).json({ success: false, message: 'Stare hasło jest nieprawidłowe' });
            }
...
exports.deleteAccount = async function (req, res) {
    ...
    try {
        ...
        if (!isMatch) {
            if (api) {
                return res.status(400).json({ success: false, message: 'Błędne hasło' });
            }
        ...
        if (api) {
            return res.status(500).json({ success: false, message: 'Nie można usunąć konta' });
        }
