...
const sendTicketEmail = async (userEmail, ticketData, pdfBuffer) => {
    try {
        const mailOptions = {
            from: `"Kino CinemaŚty" <${process.env.EMAIL_USER}>`,
            to: userEmail,
            subject: `Twój bilet: ${ticketData.movieTitle}`,
            html: `<h2>Dziękujemy za zakup!</h2><p>Bilet w załączniku.</p>`,
            attachments: [{ filename: `Bilet.pdf`, content: pdfBuffer, contentType: 'application/pdf' }]
        };
...
module.exports = { sendTicketEmail, sendResetPasswordEmail };
